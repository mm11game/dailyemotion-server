...
module.exports = {
  ...
  signup: async (req, res) => {
    ...
    if (userEmailInfo) {
      ...
      db.query(
        `INSERT INTO userInfos (user_email,password,nickName)
  ...
  change: async (req, res) => {
    ...
    if (userInfos) {
      ...
      if (nickName) {
        db.query(
          `UPDATE userInfos SET nickName = '${nickName}' WHERE user_email = '${userInfos.dataValues.user_email}'`,
          function (err, callback) {
            if (callback) {
      ...
      if (password) {
        db.query(
          `UPDATE userInfos SET password = '${password}'  WHERE user_email = '${userInfos.dataValues.user_email}'`,
          function (err, callback) {
            if (callback) {
